...
Test(ast_equal, declarations) {
    ...
    ElAstDecl* v1 = el_ast_new_var_def(&arena, span, type, &d1, false);
    ElAstDecl* v2 = el_ast_new_var_def(&arena, span, type, &d1, false);

    cr_assert(el_ast_equal_decl(v1, v2));

    // different static flag
    ElAstDecl* v3 = el_ast_new_var_def(&arena, span, type, &d1, true);
