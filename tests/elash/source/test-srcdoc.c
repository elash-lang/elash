...
Test(el_srcdoc_happy, init_append_clear) {
    ...
    assert_success(el_srcdoc_init_empty(&doc, EL_SV("test.eu")));
    cr_assert(el_sv_eql(doc.filename, EL_SV("test.eu")));
    cr_assert(!doc.is_system);
    assert_content(&doc, EL_SV(""));

    assert_success(el_srcdoc_append_str(&doc, EL_SV("hello")));
    assert_success(el_srcdoc_append_str(&doc, EL_SV(" world")));
...
Test(el_srcdoc_happy, init_from_string) {
    ...
    cr_assert_eq(el_srcdoc_init_from_str(&from_string, EL_SV("source"), EL_SV("a.eu")),
                 EL_SRCDOC_ERR_SUCCESS);

    ElStringBuf buffer;
    cr_assert(el_strbuf_init_from(&buffer, EL_SV("Keep yourself safe")));

    ElSourceDocument copied;
    assert_success(el_srcdoc_init_from_strbuf(&copied, &buffer, EL_SV("b.eu")));
...
Test(el_srcdoc_happy, copy_move_and_concat) {
    ...
    assert_success(el_srcdoc_init_from_str(&first, EL_SV("one"), EL_SV("one.eu")));
    assert_success(el_srcdoc_init_from_str(&second, EL_SV("two"), EL_SV("two.eu")));

    assert_success(el_srcdoc_copy(&first, &copy));
    cr_assert(el_sv_eql(copy.filename, EL_SV("one.eu")));
    assert_success(el_srcdoc_append_str(&first, EL_SV("!")));
    ...
    assert_success(el_srcdoc_concat(&first, &second, &concated, EL_SV("joined.eu")));
...
Test(el_srcdoc_happy, append_token_raw_forms) {
    ElSourceDocument doc;
    assert_success(el_srcdoc_init_empty(&doc, EL_SV("tokens.eu")));
    ...
    assert_success(el_srcdoc_append_token(&doc, &ident));
    assert_success(el_srcdoc_append_token(&doc, &string));
    assert_success(el_srcdoc_append_token(&doc, &comment));
    assert_success(el_srcdoc_append_token(&doc, &newline));
