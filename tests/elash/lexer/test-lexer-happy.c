...
Test(elash_lexer_happy, basic_tokens) {
    ...
    el_srcdoc_init_from_str(&doc, EL_SV("foo bar 123 3.14 'a' \"hello\""), EL_SV("test.ela"));
...
Test(elash_lexer_happy, keywords) {
    ElSourceDocument doc;
    el_srcdoc_init_from_str(&doc, EL_SV("if else while for return break continue"), EL_SV("test.ela"));
...
Test(elash_lexer_happy, operators) {
    ElSourceDocument doc;
    el_srcdoc_init_from_str(&doc, EL_SV("+ - * / % ++ -- = += -= *= /= %= == != < <= > >= && || ! & | ^ ~ &= |= ^= << >> <<= >>="), EL_SV("test.ela"));
...
Test(elash_lexer_happy, comments_kept) {
    ElSourceDocument doc;
    el_srcdoc_init_from_str(&doc, EL_SV("foo // line comment\nbar /* block comment */ baz"), EL_SV("test.ela"));
