...
#define TYPED_INT_RET(type, val)    el_hir_new_int_constant(binder->hir_arena, type, val);
#define TYPED_CHAR_RET(type, val)   el_hir_new_char_constant(binder->hir_arena, type, val);
#define TYPED_BOOL_RET(type, val)   el_hir_new_bool_constant(binder->hir_arena, type, val);

#define UNTYPED_INT_RET(type, val)  el_hir_new_untyped_int_lit(binder->hir_arena, val);
#define UNTYPED_CHAR_RET(type, val) el_hir_new_untyped_char_lit(binder->hir_arena, val);
#define UNTYPED_BOOL_RET(type, val) el_hir_new_untyped_bool_lit(binder->hir_arena, val);

// TODO: we probably should report an error on division/modulo by zero instead of returning NULL
#define ARITH_BW_BIN_OP_CASES(a, b, RET_MACRO, type)                                          \
    case EL_SEMA_BIN_OP_ADD:    return RET_MACRO(type, (a) + (b));                            \
    case EL_SEMA_BIN_OP_SUB:    return RET_MACRO(type, (a) - (b));                            \
    case EL_SEMA_BIN_OP_MUL:    return RET_MACRO(type, (a) * (b));                            \
    case EL_SEMA_BIN_OP_DIV:    if ((b) == 0) return NULL; return RET_MACRO(type, (a) / (b)); \
    case EL_SEMA_BIN_OP_MOD:    if ((b) == 0) return NULL; return RET_MACRO(type, (a) % (b)); \
    case EL_SEMA_BIN_OP_BW_AND: return RET_MACRO(type, (a) & (b));                            \
    case EL_SEMA_BIN_OP_BW_OR:  return RET_MACRO(type, (a) | (b));                            \
    case EL_SEMA_BIN_OP_BW_XOR: return RET_MACRO(type, (a) ^ (b));                            \
    case EL_SEMA_BIN_OP_BW_IMP: return RET_MACRO(type, ~(a) | (b));                           \
    case EL_SEMA_BIN_OP_SHL:    return RET_MACRO(type, (a) << (b));                           \
    case EL_SEMA_BIN_OP_SHR:    return RET_MACRO(type, (a) >> (b));

#define COMP_BIN_OP_CASES(a, b, RET_BOOL, type)                 \
    case EL_SEMA_BIN_OP_EQ:  return RET_BOOL(type, (a) == (b)); \
    case EL_SEMA_BIN_OP_NEQ: return RET_BOOL(type, (a) != (b)); \
    case EL_SEMA_BIN_OP_LT:  return RET_BOOL(type, (a) < (b));  \
    case EL_SEMA_BIN_OP_LTE: return RET_BOOL(type, (a) <= (b)); \
    case EL_SEMA_BIN_OP_GT:  return RET_BOOL(type, (a) > (b));  \
    case EL_SEMA_BIN_OP_GTE: return RET_BOOL(type, (a) >= (b));

#define BOOL_BIN_OP_CASES(a, b, RET_BOOL, type)                 \
    case EL_SEMA_BIN_OP_EQ:  return RET_BOOL(type, (a) == (b)); \
    case EL_SEMA_BIN_OP_NEQ: return RET_BOOL(type, (a) != (b)); \
    case EL_SEMA_BIN_OP_AND: return RET_BOOL(type, (a) && (b)); \
    case EL_SEMA_BIN_OP_OR:  return RET_BOOL(type, (a) || (b)); \
    case EL_SEMA_BIN_OP_IMP: return RET_BOOL(type, !(a) || (b));

#define UNARY_INT_OP_CASES(a, RET_MACRO, type)                  \
    case EL_SEMA_UNARY_OP_POS:    return RET_MACRO(type, +(a)); \
    case EL_SEMA_UNARY_OP_NEG:    return RET_MACRO(type, -(a)); \
    case EL_SEMA_UNARY_OP_BW_NOT: return RET_MACRO(type, ~(a));

#define UNARY_BOOL_OP_CASES(a, RET_BOOL, type) \
    if (op == EL_SEMA_UNARY_OP_NOT) {          \
        return RET_BOOL(type, !(a));           \
    }
...
#define FOLD_BINARY(KIND, T, MEMBER, TYPED_RET, UNTYPED_RET) \
    case EL_PRIMTYPE_##KIND: {                               \
        ...
        switch (op) {                                        \
        ARITH_BW_BIN_OP_CASES(a, b, TYPED_RET, lhs->type)    \
        COMP_BIN_OP_CASES(a, b, UNTYPED_BOOL_RET, NULL)      \
        default: return NULL;                                \
        }                                                    \
    }
...
#define FOLD_UNARY(KIND, T, MEMBER, TYPED_RET, UNTYPED_RET) \
    case EL_PRIMTYPE_##KIND: {                              \
        ...
        switch (op) {                                       \
        UNARY_INT_OP_CASES(a, TYPED_RET, operand->type)     \
        default: return NULL;                               \
        }                                                   \
    }
...
#define FOLD_BINARY_UNTYPED(KIND, T, MEMBER, TYPED_RET, UNTYPED_RET)        \
    if (lkind == EL_HIR_UNTYPED_##KIND && rkind == EL_HIR_UNTYPED_##KIND) { \
        ...
        switch (op) {                                                       \
        ARITH_BW_BIN_OP_CASES(a, b, UNTYPED_RET, NULL)                      \
        COMP_BIN_OP_CASES(a, b, UNTYPED_BOOL_RET, NULL)                     \
        default: return NULL;                                               \
        }                                                                   \
    }
...
#define FOLD_UNARY_UNTYPED(KIND, T, MEMBER, TYPED_RET, UNTYPED_RET) \
    if (kind == EL_HIR_UNTYPED_##KIND) {                            \
        ...
        switch (op) {                                               \
        UNARY_INT_OP_CASES(a, UNTYPED_RET, NULL)                    \
        default: return NULL;                                       \
        }                                                           \
    }
...
static ElHirExpr* apply_binary_operator(ElBinder* binder, ElHirExpr* lhs, ElSemaBinOp op, ElHirExpr* rhs) {
    switch (lhs->type->as.prim.kind) {
        ...
        case EL_PRIMTYPE_BOOL: {
            ...
            switch (op) {
            BOOL_BIN_OP_CASES(a, b, TYPED_BOOL_RET, binder->builtins->type_bool);
            default: return NULL;
            }
        }
        default: return NULL;
...
static ElHirExpr* apply_unary_operator(ElBinder* binder, ElSemaUnaryOp op, ElHirExpr* operand) {
    ...
    case EL_PRIMTYPE_BOOL: {
        ...
        UNARY_BOOL_OP_CASES(a, TYPED_BOOL_RET, binder->builtins->type_bool);
        return NULL;
    }
    default: return NULL;
    }
...
static ElHirExpr* apply_binary_operator_untyped(ElBinder* binder, ElHirExpr* lhs, ElSemaBinOp op, ElHirExpr* rhs) {
    ...
    if (lkind == EL_HIR_UNTYPED_BOOL && rkind == EL_HIR_UNTYPED_BOOL) {
        ...
        switch (op) {
        BOOL_BIN_OP_CASES(a, b, UNTYPED_BOOL_RET, NULL);
        default: return NULL;
        }
    }
    return NULL;
...
static ElHirExpr* apply_unary_operator_untyped(ElBinder* binder, ElSemaUnaryOp op, ElHirExpr* operand) {
    ...
    if (kind == EL_HIR_UNTYPED_BOOL) {
        ...
        UNARY_BOOL_OP_CASES(a, UNTYPED_BOOL_RET, NULL);
    }
    return NULL;
}
...
ElHirExpr* _el_binder_simplify_expr(ElBinder* binder, ElHirExpr* expr) {
    ...
    default:
        // TODO: maybe we can iterate over all elements in array constant of all arguments in function call and call simplify on it? but i'm too lazy to do it so
        return expr;
    }
}
