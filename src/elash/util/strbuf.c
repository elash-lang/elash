...
void el_strbuf_init(ElStringBuf* sb) {
    el_strbuf_init_with_cap(sb, EL_STRBUF_DEFAULT_CAP);
}
...
void el_strbuf_init_from_cstr(ElStringBuf* sb, const char* cstr) {
    el_strbuf_init_from(sb, el_sv_from_cstr(cstr));
}
...
void el_strbuf_resize(ElStringBuf* sb, usize new_len) {
    if (new_len > sb->cap) {
        el_strbuf_reserve_exact(sb, new_len);
    }
    sb->len = new_len;
}
...
void el_strbuf_reserve(ElStringBuf* sb, usize min_cap) {
    ...
    sb->data = el_realloc(sb->data, new_cap, 1);
    sb->cap = new_cap;
}
...
void el_strbuf_append(ElStringBuf* sb, ElStringView sv) {
    if (sv.len == 0) return;
    el_strbuf_reserve(sb, sb->len + sv.len);
...
void el_strbuf_append_char(ElStringBuf* sb, char c) {
    el_strbuf_reserve(sb, sb->len + 1);
    sb->data[sb->len] = c;
    sb->len += 1;
}

void el_strbuf_appendf(ElStringBuf* sb, const char* fmt, ...) {
    va_list args;
    va_start(args, fmt);
    ...
    el_strbuf_reserve(sb, sb->len + needed + 1);
