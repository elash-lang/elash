...
void el_ast_dump_decl(ElAstDecl* node, usize indent, FILE* out) {
    switch (node->type) {
    case EL_AST_DECL_VAR:
        el_ast_dump_print_indent(indent, out);
        fprintf(out, "VarDef:\n");
        el_ast_dump_print_indent(indent + 1, out);
        fprintf(out, "type:\n");
        el_ast_dump_type(node->as.var.type, indent + 2, out);
        el_ast_dump_print_indent(indent + 1, out);
        fprintf(out, "name: " EL_SV_FMT "\n", EL_SV_FARG(node->as.var.name->name));
        if (node->as.var.init != NULL) {
            el_ast_dump_print_indent(indent + 1, out);
            fprintf(out, "init:\n");
            el_ast_dump_init(node->as.var.init, indent + 2, out);
        }
        break;
